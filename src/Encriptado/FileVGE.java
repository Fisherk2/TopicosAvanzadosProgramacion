...
/**
 * Clase que genera un fichero .VGE que ademas actualiza y lee su contenido.
 *
 * @author Fisherk2
 */
public class FileVGE {
    ...
    private void creacionFichero() {
        try {
            vge = new File("C:\\Users\\msi\\Desktop\\ITL\\5to semestre\\Topicos Avanzados de programacion\\Topicos Programacion\\Encypt.vge");
            if (vge.createNewFile()) {//Si el archivo a sido creado, enviara un mensaje
                System.out.println("\n---------------------------\n"
                        + "El archivo VGE se ha creado exitosamente");
            }
    ...
    public void escribirFichero(String entrada) {
        try {
            FileWriter escribir = new FileWriter(vge);
    ...
    public String leerFichero() {
        ...
        String contenido = "";
        try {
            FileReader lector = new FileReader(vge);
